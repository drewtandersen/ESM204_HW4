...
The SCC is equal to `r scc_df[scc_df$rate == 3.0,]` dollars USD when using the Ramsey Rule to set the discount rate.
...
warming$policy_A_1_dam_npv <- discount_fun(2, warming$year - 2022, warming$policy_A_1_dam)
warming$policy_A_2_dam_npv <- discount_fun(2, warming$year - 2022, warming$policy_A_2_dam)
warming$policy_B_dam_npv <- discount_fun(2, warming$year - 2022, warming$policy_B_dam)
...
#policy_plot$b_cross<- B_sum*rep(1,length(policy_plot$imp_cost))

#plotting policies
ggplot(data = policy_plot) +
geom_line(aes(x = imp_cost, y = pol_a), color = "red") +
  geom_line(aes(x = imp_cost, y = pol_b), color = "blue") +
...
**Figure 6:** The red line represents the total cost of policy A that involves no implementation cost. The blue line represents the total cost of policy B at different implementation costs.

The two lines intersect at an implementation cost of $`r round(intersection_trill,0)` trillion USD. This means that Congress should pursue Policy B and spend up to this amount on implementation before Policy A becomes the cheaper option 

##Question 6
A risk averse society would choose Policy B because their is less uncertainty surrounding the possible damages than in Policy A. There is a 50% chance that a 1.5˚C warming scenario could occur for option A, which would result in even greater damages than plotted above. Given that society is risk averse, my advice to Congress would still be in favor of Policy B, even after the $`r round(intersection_trill,0)` trillion dollar implementation value is surpassed. 
